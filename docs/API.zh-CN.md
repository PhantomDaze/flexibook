...
快速上手见 [`../README.zh-CN.md`](../README.zh-CN.md) / [`../README.md`](../README.md)。文档索引见 [`README.md`](./README.md)。
...
- 内置字体资源：`assets/flexibook/font/`（`default.json`、unihex ZIP、`LICENSE-unifont.txt`）。策略摘要以本节为准。
...
| `assets/flexibook/textures/item/flexi_book.png` | **物品图标**（`flexibook:flexi_book`，模型 `layer0` → `flexibook:item/flexi_book`） |

自定义主题可把 `book_texture` 指到你自己的路径。翻页按钮使用原版 GUI，无需自定义 widgets 贴图。

**物品图标**：物品仍是 `flexibook:flexi_book`（资源包不能注册新物品）。换图标 = 用资源包覆盖  
